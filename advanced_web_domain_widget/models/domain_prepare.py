...
def compute_domain(domain_tuple, model):
    ...
    for field in left_value_split_list:
        left_user = False
        left_company = False
        model_field = field_obj.search(
            [("model_id.model", "=", model_obj), ("name", "=", field)], limit=1
        )
    ...
    if isinstance(domain, tuple) or isinstance(domain, list):
        ...
        if val == "this_quarter":
            start_of_quarter = datetime(
                current_date.year, ((current_date.month - 1) // 3) * 3 + 1, 1
            )
        ...
        if val == "this_year":
            ...
            return [
                "&",
                (field_name, ">=", start_of_year.strftime(date_format)),
                (field_name, "<", end_of_year.strftime(date_format)),
            ]
        ...
        if val == "last_month":
            start_of_last_month = (current_date - relativedelta(months=1)).replace(
                day=1
            )
        ...
        if val == "last_quarter":
            start_of_this_quarter = datetime(
                current_date.year, ((current_date.month - 1) // 3) * 3 + 1, 1
            )
            end_of_last_quarter = start_of_this_quarter
            start_of_last_quarter = (
                end_of_last_quarter - relativedelta(months=3)
            ).replace(day=1)
            return [
                "&",
        ...
        if val == "next_week":
            start_of_next_week = current_date + timedelta(
        ...
        if val == "next_month":
            start_of_next_month = (current_date + relativedelta(months=1)).replace(
                day=1
            )
            end_of_next_month = (start_of_next_month + relativedelta(months=1)).replace(
                day=1
            )
            return [
                "&",
        ...
        if val == "next_quarter":
            end_of_quarter = datetime(
                current_date.year, (((current_date.month - 1) // 3) * 3 + 3) + 1, 1
            )
            start_of_next_quarter = end_of_quarter
            end_of_next_quarter = (
                start_of_next_quarter + relativedelta(months=3)
            ).replace(day=1)
            return [
                "&",
