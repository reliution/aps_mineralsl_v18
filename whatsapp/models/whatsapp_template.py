...
class WhatsAppTemplate(models.Model):
    ...
    def _get_model_selection(self):
        #Available models are all models, as even transient models could have templates associated (e.g. payment.link.wizard)
        return [
    ...
    def _get_template_vals_from_response(self, remote_template_vals, wa_account):
        #Get dictionary of field: values from whatsapp template response json. Relational fields will use arrays instead of commands.
    ...
    def _generate_attachment_from_report(self, record=False):
        ...
        if record and self.header_type == 'document' and self.report_id:
            if record._name == 'policy.management' and record.upload_pdf:
                decoded_pdf = base64.b64decode(record.upload_pdf)
                return self.env['ir.attachment'].create({'name': str(record.name)+'.pdf','raw': decoded_pdf,'mimetype': 'application/pdf',})
