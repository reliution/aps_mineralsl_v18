                    ...
                    for obj_group in object_groups:
                        for group in obj_group:
                            ...
                            if (
                                ...
                                and group.get("string", False)
                                and group.get("context", False)
                            ):
                                domain = [
                        ...
                        for filter in object_filters:
                            # Filters By records
                            if (
                                filter.get("name", False)
                                and filter.get("string", False)
                                and (
                                    not (
                                        filter.get("invisible", False) == "1"
                                        or filter.get("invisible", False) == 1
                                    )
                                )
                                and (not filter.get("context", False))
                            ):
                                domain = [
...
class store_model_nodes(models.Model):
    ...
    model_id = fields.Many2one(
        "ir.model", string="Model", index=True, ondelete="cascade", required=True
    )
    node_option = fields.Selection(
        [("filter", "Filter"), ("group", "Groups")], string="Node Option", required=True
    )
