...
class BaseModel(models.AbstractModel):
                            ...
                            if isinstance(dom_tuple, tuple):
                                ...
                                for field in left_value_split_list:
                                    ...
                                    if field_type in [
                                        ...
                                        field_relation = model_obj.fields_get()[field][
                                            "relation"
                                        ]
    ...
    def _display_access_management_error(self, mode=None, rule=None):
        if mode and rule:
            ...
            resolution_info = _(
                "Check Applied Rule on Access Management:\n %(access_name)s",
                access_name=rule,
            )
...
{resolution_info}""".format(
    ...
    def unlink(self):
        value = (
            ...
            .sudo()
            .search([("key", "=", "uninstall_simplify_access_management")], limit=1)
            .value
        )
        if not value:
            for rec in self:
                if rec._name:
                    access_domain_ah_ids = rec._get_access_management_domain_record(
                        model=rec._name
                    )
                    if access_domain_ah_ids:
                        access_domain_ah_ids = access_domain_ah_ids.filtered(
                            lambda line: self.env.company
                            in line.access_management_id.company_ids
                        )
                    if access_domain_ah_ids:
                        flag = rec._check_access_management_right(
                            mode="unlink", records=access_domain_ah_ids
                        )
                        unlink_flag = flag["access_flag"]
                        access_rule = flag["access_rule"]
                        if not unlink_flag:
                            rec._display_access_management_error(
                                mode="unlink", rule=access_rule
                            )
        ...
        if not value:
            for rec in self:
                if rec._name:
                    ...
                    if access_domain_ah_ids:
                        flag = rec._check_access_management_right(
                            mode="write", records=access_domain_ah_ids
                        )
                        create_flag = rec._check_access_management_right(
                            mode="create", records=access_domain_ah_ids
                        )
                        write_flag = flag["access_flag"]
                        access_rule = flag["access_rule"]
                        if not write_flag and not create_flag:
                            rec._display_access_management_error(
                                mode="write", rule=access_rule
                            )
    ...
    def _get_view(self, view_id=None, view_type="form", **options):
        ...
        if view_type == "form":
            ...
            if (
                access_management_id
                and access_management_id.is_apply_on_without_company
            ):
                ...
                if (
                    hide_chatter_id
                    and hide_chatter_id.access_management_id.is_apply_on_without_company
                ):
                    for chatter_path in arch.xpath("//div[@class='oe_chatter']"):
                        chatter_path.getparent().remove(chatter_path)

                elif (
                    self.env.company in hide_chatter_id.access_management_id.company_ids
                ):
                    for chatter_path in arch.xpath("//div[@class='oe_chatter']"):
                        chatter_path.getparent().remove(chatter_path)
        ...
        if view_type in ["kanban", "tree"]:
            ...
            restrict_import = restrict_import.filtered(
                lambda x: x.is_apply_on_without_company
                or self.env.company.id in x.company_ids.ids
            )
            if (
                access_model_recs.filtered(lambda x: x.restrict_import)
                or restrict_import
            ):
                doc = arch
            ...
            if (
                access_model_recs.filtered(lambda x: x.restrict_export)
                or restrict_export
            ):
                doc = arch
