    ...
    def write(self, vals):
        ...
        for access in self.access_management_ids:
            if self.env.company in access.company_ids and access.readonly:
                if self.has_group("base.group_system") or self.has_group(
    ...
    def create(self, vals_list):
        ...
        for record in self:
            for access in record.access_management_ids:
                if self.env.company in access.company_ids and access.readonly:
                    if record.has_group("base.group_system") or record.has_group(
                        "base.group_erp_manager"
                    ):
                        raise UserError(
    ...
    def _login(cls, db, credential, user_agent_env):
        ...
        try:
            with cls.pool.cursor() as cr:
                ...
                if (
                    access_management_obj.sudo()
                    .search(
                        [
                            ("user_ids", "in", res.get("uid")),
                            ("disable_login", "=", True),
                        ]
                    )
                    .id
            ...
            _logger.info(
                "Login failed for db:%s login:%s from ", db, credential["login"]
            )
            raise
        return res
