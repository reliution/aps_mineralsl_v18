...
class access_management(models.Model):
    ...
    access_domain_ah_ids = fields.One2many(
        "access.domain.ah", "access_management_id", "Access Domain", copy=True
    )
    hide_view_nodes_ids = fields.One2many(
        "hide.view.nodes", "access_management_id", "Button/Tab Access", copy=True
    )
    ...
    # Chatter
    hide_chatter_ids = fields.One2many(
        "hide.chatter", "access_management_id", "Hide Chatters", copy=True
    )
    ...
    hide_export = fields.Boolean(
        help="The Export button will be hidden in all model from the specified users."
    )
    hide_import = fields.Boolean(
        help="The Import button will be hidden in all model from the specified users."
    )
    hide_spreadsheet = fields.Boolean()
    hide_add_property = fields.Boolean()
    disable_login = fields.Boolean(
        "Disable Login", help="The Users can not login if this button is chek."
    )
            ...
            if record.readonly:
                for user in record.user_ids:
                    if user.has_group("base.group_system") or user.has_group(
                        ...
                        raise UserError(
                            _("Admin user can not be set as a read-only..!")
                        )
        return res
    ...
    def get_chatter_hide_details(self, user_id, company_id, model=False):
        ...
        access_ids = self.search(
            [("user_ids", "in", user_id), ("company_ids", "in", company_id)]
        )
        for access in access_ids:
            for hide_chatter_id in access.hide_chatter_ids:
                model_list.append(hide_chatter_id.model_id)
        ...
        if model and hide_send_mail or hide_log_notes or hide_schedule_activity:
            ...
            if hide_ids:
                ...
                if hide_chatter and hide_ids.filtered(lambda x: x.hide_chatter):
                    hide_chatter = False

                if hide_log_notes and hide_ids.filtered(lambda x: x.hide_log_notes):
                    hide_log_notes = False

                if hide_schedule_activity and hide_ids.filtered(
                    lambda x: x.hide_schedule_activity
                ):
                    hide_schedule_activity = False
